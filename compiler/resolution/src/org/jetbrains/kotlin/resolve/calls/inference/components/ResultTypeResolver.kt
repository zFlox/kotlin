...
class ResultTypeResolver(
    ...
    private fun Context.isNothingNotSuitableFor(variable: TypeVariableMarker): Boolean {
        val variableDebugName = variable.freshTypeConstructor().safeAs<TypeVariableTypeConstructor>()?.debugName ?: return false
        val variableName = Name.identifier(variableDebugName)
        val isSpecialFunctionParameter = statelessCallbacks?.isSpecialFunctionTypeParameterName(variableName) ?: false
        val isBangBangParameter = isSpecialFunctionParameter && statelessCallbacks?.isExclExclTypeParameterName(variableName) ?: false
